package su.mandora.tarasande.screen.list.protocolhack

import com.viaversion.viaversion.api.protocol.version.ProtocolVersion
import su.mandora.tarasande.screen.element.ScreenBetterSlotList
import su.mandora.tarasande.screen.element.ScreenBetterSlotListEntry
import su.mandora.tarasande.screen.element.ScreenBetterSlotListWidget
...
class ScreenBetterProtocolHack(parent: Screen) : ScreenBetterSlotList(parent, 46, 12, object : ScreenBetterSlotListWidget.ListProvider {
    ...
    class ProtocolEntry(val protocol: ProtocolVersion) : ScreenBetterSlotListEntry() {
        ...
        override fun renderEntry(matrices: MatrixStack, index: Int, entryWidth: Int, entryHeight: Int, mouseX: Int, mouseY: Int, hovered: Boolean) {
            RenderUtil.textCenter(matrices, Text.literal(this.protocol.name), entryWidth.toFloat(), 0F, if (this.isSelected()) colorShift(Color.green) else colorShift(Color.red))
        }
