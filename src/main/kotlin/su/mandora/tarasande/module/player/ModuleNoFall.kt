...
class ModuleNoFall : Module("No fall", "Prevents or reduces fall damage", ModuleCategory.PLAYER) {
    ...
    private val mode = ValueMode(this, "Mode", false, "Ground spoof", "Re-Ground", "Drag down")
    ...
    val eventConsumer = Consumer<Event> { event ->
        when (event) {
            ...
            is EventUpdate -> {
                if(event.state == EventUpdate.State.POST && mode.isSelected(1))
                    if(mc.player?.fallDistance!! >= fallDistance.value) {
