...
class ElementWidthPlayer(val gameProfile: GameProfile, width: Double) : ElementWidth(width) {
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        context.fill(0.0, 0.0, width, getHeight(), Int.MIN_VALUE)
        if (friended) {
            ...
            FontWrapper.textShadow(context, gameProfile.name, xOffset.toFloat(), yOffset.toFloat(), -1)
        }
    ...
    override fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
        if (button == 0) {
            return if (RenderUtil.isHovered(mouseX, mouseY, 0.0, 0.0, width, getHeight()) && Vec2f(mouseX.toFloat(), mouseY.toFloat()).distanceSquared(Vec2f((width - 7).toFloat(), (defaultHeight / 2).toFloat())) < 16F) {
