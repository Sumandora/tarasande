...
class ScreenBetterEnvironment(prevScreen: Screen?, private val environment: Environment?, private val environmentConsumer: Consumer<Environment>) : ScreenBetter("Environment", prevScreen) {
    ...
    override fun init() {
        ...
        accountsHostTextField = TextFieldWidgetPlaceholder(this.textRenderer, this.width / 2 - TEXTFIELD_WIDTH / 2, (height * 0.25F + 1 * (DEFAULT_BUTTON_HEIGHT + BUTTON_PADDING)).toInt(), TEXTFIELD_WIDTH, DEFAULT_BUTTON_HEIGHT, Text.of("Accounts Host")).also {
            it.setMaxLength(Int.MAX_VALUE)
            it.text = environment.accountsHost
            addDrawableChild(it)
        }

        sessionHostTextField = TextFieldWidgetPlaceholder(this.textRenderer, this.width / 2 - TEXTFIELD_WIDTH / 2, (height * 0.25F + 2 * (DEFAULT_BUTTON_HEIGHT + BUTTON_PADDING)).toInt(), TEXTFIELD_WIDTH, DEFAULT_BUTTON_HEIGHT, Text.of("Session Host")).also {
            it.setMaxLength(Int.MAX_VALUE)
            it.text = environment.sessionHost
            addDrawableChild(it)
        }

        servicesHostTextField = TextFieldWidgetPlaceholder(this.textRenderer, this.width / 2 - TEXTFIELD_WIDTH / 2, (height * 0.25F + 3 * (DEFAULT_BUTTON_HEIGHT + BUTTON_PADDING)).toInt(), TEXTFIELD_WIDTH, DEFAULT_BUTTON_HEIGHT, Text.of("Services Host")).also {
            it.setMaxLength(Int.MAX_VALUE)
        ...
        this.addDrawableChild(ButtonWidget(width / 2 - DEFAULT_BUTTON_WIDTH / 2, DEFAULT_BUTTON_HEIGHT + BUTTON_PADDING + (height * 0.75F).toInt(), DEFAULT_BUTTON_WIDTH, DEFAULT_BUTTON_HEIGHT, Text.of("Done")) {
            environmentConsumer.accept(Environment(accountsHostTextField?.text!!, sessionHostTextField?.text!!, servicesHostTextField?.text!!, "Custom"))
