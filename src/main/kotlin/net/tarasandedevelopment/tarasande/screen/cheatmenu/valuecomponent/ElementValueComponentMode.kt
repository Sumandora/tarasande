...
class ElementValueComponentMode(value: Value) : ElementValueComponent(value) {
    ...
    override fun render(matrices: MatrixStack?, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        for ((index, setting) in valueMode.settings.withIndex()) {
            ...
            RenderUtil.font().textShadow(matrices, setting, (width - RenderUtil.font().getWidth(setting) / 2f).toFloat(), (getHeight() / 2.0F + (index - (valueMode.settings.size - 1) / 2.0 + 0.5) * (RenderUtil.font().fontHeight() / 2.0f) - RenderUtil.font().fontHeight() / 2.0F).toFloat(), color.rgb, scale = 0.5F, offset = 0.5F)
        }
    }
