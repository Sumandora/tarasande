package net.tarasandedevelopment.tarasande.graphsystem.panel
...
class PanelGraph(private val graph: Graph, x: Double, y: Double) : Panel(graph.name, x, y, max(100.0, FontWrapper.getWidth(graph.name) + 10.0), 50.0, fixed = true) {
    ...
    init {
        TarasandeMain.get().valueSystem.getValues(graph).forEach {
            it.owner = this
        }
    }
