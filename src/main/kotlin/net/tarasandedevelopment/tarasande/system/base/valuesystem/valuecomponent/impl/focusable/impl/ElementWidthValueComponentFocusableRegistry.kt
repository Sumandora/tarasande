...
class ElementWidthValueComponentFocusableRegistry(value: Value) : ElementWidthValueComponentFocusable<ValueRegistry<*>>(value) {
    ...
    override fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
        ...
        for ((index, key) in entries.withIndex()) {
            if (RenderUtil.isHovered(mouseX, mouseY, (width.toFloat() - FontWrapper.getWidth(StringUtil.uncoverTranslation(value.getTranslationKey(key))) / 2.0F).toDouble(), (FontWrapper.fontHeight() / 2.0F * (index + 0.5F)).toDouble(), width, (FontWrapper.fontHeight() / 2.0F * ((index + 1) + 0.5F)).toDouble())) {
                value.remove(key)
                updateSearchResults()
                return true
            }
