...
class GeneratorQRCode(parent: Any) : Generator(parent, "QR Code") {
    ...
    override fun perform() {
        try {
            writer.encode(input.value, if (type.anySelected()) BarcodeFormat.valueOf(type.selected[0]) else BarcodeFormat.QR_CODE, dimensionScaleX.value.toInt(), dimensionScaleYorZ.value.toInt()).apply {
