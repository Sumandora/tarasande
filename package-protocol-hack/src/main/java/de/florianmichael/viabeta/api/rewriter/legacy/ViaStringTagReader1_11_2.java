...
public class ViaStringTagReader1_11_2 {
    ...
    static Any joinStrToNBT(String... args) throws RuntimeException {
        return nameValueToNBT(args[0], args[1]);
    }

    static Any nameValueToNBT(String key, String value) throws RuntimeException {
        value = value.trim();
        ...
        if (value.startsWith("{")) {
            ...
            for (jsontonbt$compound = new Compound(key); value.length() > 0; value = value.substring(s1.length() + 1)) {
            ...
            for (jsontonbt$list = new List(key); value.length() > 0; value = value.substring(s.length() + 1)) {
    ...
    private static Any getTagFromNameValue(String str, boolean isArray) throws RuntimeException {
