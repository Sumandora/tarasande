...
public class XboxRequests {

	//go here, log in, and in the redirected url you will have your access token,
	...
	public XboxRequests(String accessToken) {
	...
	public String getUserToken(ECPublicKey publicKey, ECPrivateKey privateKey) throws Exception {
		...
		String responce = new String(IOUtils.toByteArray(connection.getInputStream()));
	...
	public String requestMinecraftChain(String xsts, ECPublicKey publicKey) throws Exception {
		...
		connection.setRequestProperty("Client-Version", BedrockSessionStorage.CODEC.getMinecraftVersion());
	...
	private String getProofKeyX(ECPublicKey ecPublicKey) {
		return Base64.getUrlEncoder().withoutPadding().encodeToString(XboxRequests.bigIntegerToByteArray(ecPublicKey.getW().getAffineX()));
	}

	private String getProofKeyY(ECPublicKey ecPublicKey) {
		return Base64.getUrlEncoder().withoutPadding().encodeToString(XboxRequests.bigIntegerToByteArray(ecPublicKey.getW().getAffineY()));
	}
