...
public class BedrockAuthenticator {

	public static final BedrockAuthenticator INSTANCE = new BedrockAuthenticator();
	...
	public String getOnlineChainData() throws Exception {
		...
		KeyPair ecdsa256KeyPair = BedrockAuthenticator.createKeyPair(); // for xbox live, xbox live requests use, ES256, ECDSA256
		this.publicKey = (ECPublicKey) ecdsa256KeyPair.getPublic();
		this.privateKey = (ECPrivateKey) ecdsa256KeyPair.getPrivate();

		XboxRequests xbox = new XboxRequests(System.getProperty("ViaCursedBedrock"));
		...
		JsonObject chainDataObject = new JsonParser().parse(chainData).getAsJsonObject();
		...
		String firstKeyx5u = new JsonParser().parse(firstChainHeader).getAsJsonObject().get("x5u").getAsString();
...

		{
			...
			JsonObject payloadObject = new JsonParser().parse(lastChainPayload).getAsJsonObject();
